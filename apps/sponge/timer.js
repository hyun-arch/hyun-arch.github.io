...
(() => {
  ...
  const el = {
    ...
    pinBtn: $("#pinBtn"), widgetBtn: $("#widgetBtn"), presets: $("#presets"), customChip: $("#customChip"),
  ...
  el.widgetBtn.addEventListener("click", openWidget);

  // Extension → background opens the popup widget window.
  // Standalone web/PWA → open widget.html as a small pop-up window.
  function openWidget() {
    if (SC.openSurface("widget")) return;
    window.open("widget.html", "sc-widget",
      "popup,width=268,height=216,left=" + Math.max(0, screen.availWidth - 292) + ",top=24");
  }
  ...
  document.addEventListener("keydown", (e) => {
    ...
    switch (e.key) {
      ...
      case "w": case "W": case "ㅈ": openWidget(); break;

...
function seedIfNeeded(){
  ...
  const rev=window.AHOS_SEEDREV||1;
  if(!S.get('seeded',false)){
    ...
    S.set('sopRuns',clone(window.SEED_SOP_RUNS)); S.set('seeded',true); S.set('seedRev',rev);
    return;
  }
  // 시드 개정 시: 기존 사용자 데이터는 보존하고, id 기준으로 새 시드 항목만 추가.
  if(S.get('seedRev',1) < rev){
    const topUp=(key,arr)=>{ const cur=S.get(key,[]); const ids=new Set(cur.map(x=>x.id)); let add=false;
      arr.forEach(it=>{ if(!ids.has(it.id)){ cur.push(clone(it)); add=true; } }); if(add) S.set(key,cur); };
    topUp('people',window.SEED_PEOPLE); topUp('okrs',window.SEED_OKRS);
    topUp('decisions',window.SEED_DECISIONS); topUp('sopTpl',window.SEED_SOP_TEMPLATES);
    S.set('seedRev',rev);
  }
}
// 현재 사용자(프로토타입): 첫 사람
